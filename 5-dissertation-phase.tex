...
possible: \textit{lazy reasoning} and \textit{parallel reasoning}, which
converge into the \system\ library, the deliverable of the proposal.

\subsection{Laziness}
\label{sec:lazy}

With a single structure carrying one equivalence relation per assumption
context, \textit{lazy reasoning} asks how to exploit the dependencies between
these contexts to avoid redundant work. If two equivalence relations are induced
by logically related conditions, much of one can be derived from the other.
As these equivalence relations form a lattice, two operations arise alongside the
\textit{fork} that creates a new
...
facts common to every case of a split. Versioned e-graphs already provide a fork
operation, which is a specific form of laziness that in effect clones a relation
on demand. The goal now is to exploit richer dependencies between contexts,
such as those induced by the semantics of the conditions that separate them,
so that reasoning under a condition like $a = b \to c = d$ can reuse the relation
already computed under $a = b$. Laziness reifies an inherited equality only when
queried, amortizing the cost over the queries that use it. We can
...
The lattice of equivalence relations also opens the way to a new parallelization
strategy: our extension for \textit{parallel reasoning} treats the lattice as a
dependency graph of tasks, each exploring one relation modulo its dependencies,
and asks how to schedule that graph across many workers. 
Three difficulties shape the problem.
...
\subsection{\system[\textbf]\ Library}

The \system\ library is the deliverable of the proposal: a single e-graph
implementation with all four proposed contributions. A host prover can adopt it in
place of its own structure and enable only the extensions it needs, so that our
artifact can reach the verification ecosystem rather than one tool. Our extensions can
interact in subtle ways, so their composition is not trivial. For example,
disequalities should be interpreted per version and be preserved by the
meet and join. We will develop the \system\ library incrementally, validating it against the
