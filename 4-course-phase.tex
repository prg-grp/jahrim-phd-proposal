...
\subsection{Disequalities}
To reason about mutually exclusive cases in a proof by cases, we first need to
introduce a semantic extension for \textit{disequality reasoning}.
I contributed to the work on \textit{dis/equality
graphs}~\cite{zakhour2025disequality}, which extend e-graphs with direct support
for disequality.
...
framework for reasoning about e-graphs and used it to prove that the relation an
e-graph represents coincides with the reflexive, symmetric, transitive, and
...
extension to egg~\cite{willsey2021egg} and evaluated them in an EUF solver and
...
With disequalities in place, we can now focus on a semantic extension for
\textit{conditional reasoning}, that is, reasoning about the equalities that
hold within each case.
I authored \textit{versioned e-graphs}, which extend e-graphs to represent
equalities that hold only conditionally~\cite{cesario2026versioned}.
...
branches share. A versioned e-graph instead encodes many equivalence relations, one
...
the state-of-the-art Easter Egg~\cite{singher2024easteregg} across two case
studies, an EUF
